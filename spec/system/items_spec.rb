...
RSpec.describe '商品詳細', type: :system do
  ...
  context 'ログインしているとき' do 
    context '自分が出品した商品ページのとき' do 
      context '商品が販売中のとき' do
        it 'ログインした上で自分が出品した商品の商品詳細ページへアクセスした場合は、「商品の編集」「削除」のリンクが存在すること' do
          # ログインする
          visit new_user_session_path
          fill_in 'user[email]', with: @item1.user.email
          fill_in 'user[password]', with: @item1.user.password
        ...
        it 'ログインした上で自分が出品した商品の商品詳細ページへアクセスした場合は、「購入画面に進む」のリンクが存在しないこと'do
          # ログインする
          visit new_user_session_path
          fill_in 'user[email]', with: @item1.user.email
          fill_in 'user[password]', with: @item1.user.password
      ...
      context '商品が売却済みのとき' do
        it 'ログインした上で自分が出品した商品の商品詳細ページへアクセスした場合は、「商品の編集」「削除」「購入画面に進む」のリンクが存在しないこと' do
          # ログインする
          visit new_user_session_path
          fill_in 'user[email]', with: @item1.user.email
          fill_in 'user[password]', with: @item1.user.password
          ...
          expect(page).to have_no_link '購入画面に進む', href: item_transactions_path(@item3)
        end
      end
    end
    context '自分が出品した商品ページではないとき' do
      ...
      context '商品が売却済みのとき' do
        it 'ログインした上で自分が出品していない商品の商品詳細ページへアクセスした場合は、「SoldOut」が表示され、「商品の編集」「削除」「購入画面に進む」のリンクは存在しないこと' do
          # ログインする
          visit new_user_session_path
          fill_in 'user[email]', with: @item1.user.email
          fill_in 'user[password]', with: @item1.user.password
  ...
  context 'ログインしてないとき' do
    context '商品が販売中のとき' do
      it 'ログインせずに商品詳細ページへアクセスした場合は、「商品の編集」「編集」「削除」のリンクは存在しないこと' do
        # トップページへ行く
        visit root_path
        # 商品のページへアクセスする
        find(:xpath, "//a[@href='/items/#{@item2.id}']").click
        ...
        expect(page).to have_no_link '商品の編集', href: edit_item_path(@item2)
        expect(page).to have_no_link '削除', href: item_path(@item2)
        expect(page).to have_no_link '購入画面に進む', href: item_transactions_path(@item2)
      end
    end
    context '商品が売却済みのとき' do
      it 'ログインせずに商品詳細ページへアクセスした場合は、「商品の編集」「編集」「削除」のリンクは存在しないこと' do
...
RSpec.describe '商品購入', type: :system do
  ...
  context '商品購入ができないとき' do 
    ...
    it 'ログインしていないユーザーは、商品購入画面に遷移しようとすると、ログインページへ遷移すること' do
